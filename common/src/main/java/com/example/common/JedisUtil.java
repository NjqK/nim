    ...
    public static void initJedisUtil(String redisHost, int redisPort, String redisPassword,
        ...
        if (pool == null) {
            ...
            if (redisPassword != null && !"".equals(redisPassword)) {
                // redis 设置了密码
                pool = new JedisPool(jedisPoolConfig, redisHost, redisPort, 10000, redisPassword);
            } else {
                // redis 未设置密码
                pool = new JedisPool(jedisPoolConfig, redisHost, redisPort, 10000);
            }
            log.info("init JedisUtil successfully");
        }
    }
